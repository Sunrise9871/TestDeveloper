...
namespace CookingPrototype.Controllers {
	public class CustomersController : MonoBehaviour {
		...
		public bool ServeOrder(Order order) {
			...
			var occupiedPlaces = CustomerPlaces.FindAll(x => !x.IsFree);
			foreach ( var occupied in occupiedPlaces ) {
				var customer = occupied.CurCustomer;
				var orderPlaces = customer.OrderPlaces;
				if ( orderPlaces.All(orderPlace => orderPlace.CurOrder != order) ) {
					continue;
				}

				if ( lowestTimeCustomer == null )
					lowestTimeCustomer = customer;
				else if ( customer.WaitTime < lowestTimeCustomer.WaitTime )
					lowestTimeCustomer = customer;
			}
			...
			lowestTimeCustomer.ServeOrder(order);
